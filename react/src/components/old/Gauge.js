...
function Gauge({ selection }) {
    ...
    const [maxWeight, setMaxWeight] = useState(
        Number(localStorage?.getItem('trailfeedMaxWeight')) || 96
    );

    const handleChange = (event) => {
        const newVal = Number(event.target.value.replace(',', ''));
        localStorage.setItem(event.name, newVal);
        currentStat === 0 ? setMaxCalories(newVal) : setMaxWeight(newVal);
    };
    ...
    const ref = useD3(() => {
        ...
        tracks
            ...
            .attrTween('d', (d, i, nodes) => {
                ...
                const interpolate = d3.interpolate(interpolateFrom, d.value);
            ...
            {currentStat === 2 && (
                <div className="input-container">
                    <input
                        ...
                        type="text"
                        onChange={handleChange}
                        value={maxWeight.toLocaleString()}
                        name="trailfedMaxWeight"
                    />
                    <span onClick={() => input.focus()}>Target oz.:</span>
                </div>
            )}
        </div>
    );
